...
class ConnectionHandler:
    ...
    def _initialize_intent(self):
        ...
        self.logger.bind(tag=TAG).info(f"初始化意图识别，类型: {self.intent_type}")

        if self.intent_type == "function_call" or self.intent_type == "intent_llm":
            self.load_function_plugin = True
            self.logger.bind(tag=TAG).info("已启用函数插件加载")
        """初始化意图识别模块"""
        # 获取意图识别配置
        ...
        if intent_type == "nointent":
            self.logger.bind(tag=TAG).info("使用nointent模式，跳过意图识别初始化")
            return
        # 使用 intent_llm 模式
        ...
        self.logger.bind(tag=TAG).info("已创建统一工具处理器")

        # 异步初始化工具处理器
        if hasattr(self, "loop") and self.loop:
            asyncio.run_coroutine_threadsafe(self.func_handler._initialize(), self.loop)
            self.logger.bind(tag=TAG).info("已启动工具处理器异步初始化")
