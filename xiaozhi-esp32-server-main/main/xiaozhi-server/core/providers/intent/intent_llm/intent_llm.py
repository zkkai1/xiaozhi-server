...
class IntentProvider(IntentProviderBase):
    ...
    def get_intent_system_prompt(self, functions_list: str) -> str:
        ...
        prompt = (
            ...
            "```\n"
            "用户: 现在的水温是多少？\n"
            '返回: {"function_call": {"name": "get_temperature"}}\n'
            "```\n"
            "```\n"
            "用户: 水杯温度怎么样？\n"
            '返回: {"function_call": {"name": "get_temperature"}}\n'
    ...
    async def detect_intent(self, conn, dialogue_history: List[Dict], text: str) -> str:
        ...
        logger.bind(tag=TAG).info(f"待识别文本: {text}")
        ...
        # 强制清除意图缓存，确保使用新的系统提示词
        if hasattr(self, 'promot') and self.promot:
            self.intent_cache.clear()
            logger.bind(tag=TAG).info("已清除意图缓存，确保使用新的系统提示词")

        # 强制重新生成系统提示词，确保包含最新的温度查询示例
        logger.bind(tag=TAG).info("强制重新生成系统提示词...")
        ...
        logger.bind(tag=TAG).info(f"可用的函数列表: {[f.get('function', {}).get('name') for f in functions]}")
            self.promot = self.get_intent_system_prompt(functions)
        logger.bind(tag=TAG).info("系统提示词已重新生成")
        ...
        logger.bind(tag=TAG).debug(f"意图识别系统提示词: {prompt_music}")
        ...
        logger.bind(tag=TAG).info(f"LLM原始返回: {intent}")
        ...
        if match:
            ...
            logger.bind(tag=TAG).info(f"提取的JSON意图: {intent}")
        ...
        try:
            ...
            if "function_call" in intent_data:
                ...
                # 记录识别到的function call
                logger.bind(tag=TAG).info(
                    f"最终识别到的意图: {function_name}, 参数: {function_args}"
                )
